...
function AddEditOrder() {
  ...
  useEffect(() => {
    if (id !== '0') {
      setEdit(true);
      Axios.get(`https://fractal-technical-test-backend.vercel.app/orders/${id}`).then((response) => {
  ...
  const handleSubmit = (event) => {
    ...
    if (edit) {
      Axios.put(`https://fractal-technical-test-backend.vercel.app/update/${id}`, orderData).then(() => {
        Swal.fire({
      ...
      Axios.post('https://fractal-technical-test-backend.vercel.app/create', orderData).then(() => {
        Swal.fire({
